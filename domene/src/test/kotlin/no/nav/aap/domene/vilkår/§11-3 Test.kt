...
internal class `§11-3 Test` {
    ...
    fun `Hvis saksbehandler manuelt har oppfylt 11-3, settes vilkår til oppfylt`() {
        ...
        val løsning = LøsningParagraf_11_3(UUID.randomUUID(), "saksbehandler", LocalDateTime.now(), true)
    ...
    fun `Hvis saksbehandler manuelt IKKE har oppfylt 11-3, settes vilkår til ikke oppfylt`() {
        ...
        val løsning = LøsningParagraf_11_3(UUID.randomUUID(), "saksbehandler", LocalDateTime.now(), false)
        vilkår.håndterLøsning(løsning)
    ...
    fun `Hvis tilstand oppfylt blir godkjent av kvalitetssiker blir tilstand satt til oppfylt kvalitetssikret`() {
        ...
        val kvalitetssikring = KvalitetssikringParagraf_11_3(UUID.randomUUID(), UUID.randomUUID(), "X", LocalDateTime.now(), true, "JA")
    ...
    fun `Hvis tilstand oppfylt ikke blir godkjent av kvalitetssiker blir tilstand satt tilbake til avventer manuell vurdering`() {
        ...
        val kvalitetssikring = KvalitetssikringParagraf_11_3(UUID.randomUUID(), UUID.randomUUID(), "X", LocalDateTime.now(), false, "NEI")
        vilkår.håndterKvalitetssikring(kvalitetssikring)
    ...
    private fun assertUtfall(utfall: Utfall, vilkårsvurdering: Paragraf_11_3) {
        assertEquals(utfall, listOf(vilkårsvurdering).toDto().first().utfall)
    }

    private fun assertTilstand(tilstand: Vilkårsvurdering.Tilstand.Tilstandsnavn, vilkårsvurdering: Paragraf_11_3) {
        assertEquals(tilstand.name, listOf(vilkårsvurdering).toDto().first().tilstand)
    }

    private fun assertKvalitetssikretAv(kvalitetssikretAv: String, vilkårsvurdering: Paragraf_11_3) {
        assertEquals(kvalitetssikretAv, listOf(vilkårsvurdering).toDto().first().kvalitetssikretAv)
    }

    private fun assertIkkeKvalitetssikret(vilkårsvurdering: Paragraf_11_3) {
